...
def tab_insights(tab):
    with tab:
        ...
        # Ambil semua insights dari database global
        insights = get_custom_insights()
        
        # Tampilkan insights yang diambil dari database
        if insights:
            ...
            selected_tags = st.multiselect("Filter berdasarkan Tag", list(all_tags), key="filter_db")
            
            for insight in insights:
                if not selected_tags or any(tag in selected_tags for tag in insight.get('tags', [])):
                    with st.expander(f"{insight.get('title', 'Untitled')} - {insight.get('created_at', 'No date')}"):
                        st.markdown("**Deskripsi:**")
                        st.write(insight.get('content', 'No content'))
                        if insight.get('tags'):
                            st.markdown(f"**Tags:** {', '.join(insight['tags'])}")
                        st.caption(f"Ditambahkan pada: {insight.get('created_at', 'No date')}")
            ...
            for insight in st.session_state.custom_insights:
                all_tags.update(insight.get('tags', []))
            selected_tags = st.multiselect("Filter berdasarkan Tag", list(all_tags), key="filter_session")
